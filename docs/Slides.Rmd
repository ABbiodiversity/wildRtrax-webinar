...
```{r}
eh14_raw %>% select(1:15) %>% glimpse(width = 90) #<<
```
...
Most of the time, there are additional steps we want to take to transform our tag data into something more insightful or useful - for, say, modeling.
...
glimpse(eh14_summarised, width = 90)
...
raw_data <- wt_get_download_summary("CAM") %>%
  filter(project == "ABMI Ecosystem Health 2014") %>%
  select(project_id) %>%
  pull() %>%
...
```
**Almost three quarters of a million tasks!**
...
+ If you think you've found a bug, or would like to suggest a new feature/function, please file an [Issue](https://github.com/ABbiodiversity/wildRtrax/issues) on the package's Github repository
...
+ We welcome these at any time!

+ If you have an idea for a function, feel free to [Fork](https://docs.github.com/en/get-started/quickstart/fork-a-repo) the wildRtrax repo, code it up, and [submit a PR](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request) for us to look at. 

+ You can do this for minor fixes to existing functions/text, too. 

+ You'll be acknowledged as a package author!

--
<br>

[Quick Demonstration ...](https://github.com/ABbiodiversity/wildRtrax)
